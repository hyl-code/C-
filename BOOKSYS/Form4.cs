...
namespace BOOKSYS
{
    public partial class Form4 : Form
    {
        ...
        {
            // TODO: 这行代码将数据加载到表“mBOOKDataSet2.TReader”中。您可以根据需要移动或移除它。
            this.tReaderTableAdapter1.Fill(this.mBOOKDataSet2.TReader);
        ...
        private void button1_Click(object sender, EventArgs e)          //读者添加
        {
            ...
            {
                sqlStr = "insert [TReader]([ReaderID],[Name],[Sex],[Spec],[Born],[Photo])" +
                    "values(@ReaderID, @Name, @Sex, @Spec, @Born, @Photo)";     //设置Sql语句
            }
            else                        //如果没选照片
            {
                sqlStr = "insert [TReader]([ReaderID],[Name],[Sex],[Spec],[Born])" +
                    "values(@ReaderID, @Name, @Sex, @Spec, @Born)";     //设置Sql语句
            }
        ...
        private void button4_Click(object sender, EventArgs e)          //读者查询
        {
            ...
            SqlConnection conn = new SqlConnection(strcon);
            string sqlStrSelect = "select [ReaderID],[Name],[Sex],[Spec],[Born],[Photo],[Num],[Photo] from [TReader] where [ReaderID]='" + textBox1
                .Text.Trim() + "'";
        ...
        private void button5_Click(object sender, EventArgs e)          //选择照片
        {
